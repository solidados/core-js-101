...
const encodeToRot13 = (str) => {
  ...
  const lastHalf = 'NOPQRSTUVWXYZnopqrstuvwxyz';
  let result = '';

  for (let i = 0; i < str.length; i += 1) {
    if (firstHalf.includes(str[i])) {
      result += lastHalf[firstHalf.indexOf(str[i])];
    } else if (lastHalf.includes(str[i])) {
      result += firstHalf[lastHalf.indexOf(str[i])];
    } else {
      result += str[i];
    }
  }
...
// console.log(encodeToRot13('uryyb'));
